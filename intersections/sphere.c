...
/*   Updated: 2023/10/26 16:24:21 by toshsharma       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_ray ray, double *t)
{
	...
	{
		dot_prod = max_num(0, dot(ray.normal, norm_light));
		...
		put_pixel(&rt->img, ray.x, ray.y,
			c_c(rt, sphere.color, dist(rt, light) * lamb_f, dist(rt, light)
				* spec(rt, ray.normal, ray, t)));
	}
}
		...
		double *t)
{
	...
	else
	{
		put_pixel(&rt->img, ray.x, ray.y, c_c(rt, sphere.color,
				dist(rt, light) * max_num(0, dot(normal,
						norm_light)), dist(rt, light)
	...
	t_quadratic *quad)
{
	...
	if (dot(dist, dist) < (sphere.diameter / 2)
...
void	intersect_sphere(t_rt *rt, t_sphere sphere, t_ray ray, double *t)
{
	...
	quad.b = 2 * dot(ray.direction, vec_subtract(ray.origin,
				sphere.center));
	quad.c = dot(vec_subtract(ray.origin, sphere.center),
