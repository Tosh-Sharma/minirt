...
/*   Updated: 2023/10/08 14:31:01 by toshsharma       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		double *t)
{
	...
	light = normalize_vector(vec_subtract(vec_add(ray.origin,
					scalar_product(ray.direction, *t)), rt->light->origin));
	lambertian_reflection = max_num(0.0, dot_product(normal, light));
...
void	intersect_sphere(t_rt *rt, t_sphere sphere, t_ray ray, double *t)
{
	...
	{
		new_t = min_num((-b + sqrt(discriminant)) / (2.0 * a),
